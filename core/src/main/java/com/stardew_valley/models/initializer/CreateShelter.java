...
import com.stardew_valley.models.animal.AnimalHouseType;
...
public class CreateShelter {
    public static AnimalHouse createShelter(Position position, Farm farm, AnimalHouseType animalHouseType) {
        ...
        setFenceX(position.x() - 1, position.x() + animalHouseType.getSize().getWidth(),
                farm, position.y() - 1, false);
        setFenceX(position.x() - 1, position.x() + animalHouseType.getSize().getWidth() + 1,
                farm, position.y() + animalHouseType.getSize().getHeight() + 1, true);
        setFenceY(position.y() - 1, position.y() + animalHouseType.getSize().getHeight() + 1,
        ...
        for (int i = position.x(); i <= end.x(); i++) {
            for (int j = position.y(); j <= end.y(); j++) {
                Tile tile = new Tile.Builder()
                        ...
                        .setObject(null)
                        .build();
                farm.getTiles().get(j).set(i, tile);
            }
        }
    }
    ...
    private static void setFenceX(int start, int end, Farm farm, int constantVar, boolean hasDoor) {
        for (int i = start; i <= end; i++) {
            Tile tile = new Tile.Builder()
                    ...
                    .setObject(null)
                    .build();
            if (!hasDoor || (i != (start + end) / 2)) {
                farm.getTiles().get(constantVar).set(i, tile);
            }
        }

    }

    private static void setFenceY(int start, int end, Farm farm, int constantVar, boolean hasDoor) {
        for (int i = start; i <= end; i++) {
            Tile tile = new Tile.Builder()
                    .setPosition(new Position(i, constantVar))
                    .setType(TileType.FENCE)
                    ...
                    .setObject(null)
                    .build();
            farm.getTiles().get(i).set(constantVar, tile);
